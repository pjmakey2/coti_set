...
def random_numeric_data(rr, start, end, ntype=float, unique: bool = False) -> list:
    logging.info(
        f"Generate {rr} of fake data from start {start} to end {end} of type {ntype} that is {unique} unique"
    )
    dd = []
    for i in range(rr):
        dd.append(ntype(uniform(start, end)))
    if unique:
        return list(set(dd))
    return dd
...
parser.add_argument(
    ...
    action="store_true",
    default=False,
    help="Remove and initialize the test atabase",
)
parser.add_argument(
    "--populate_db",
    action="store_true",
...
parser.add_argument(
    "--n_values", nargs="*", type=int, help="Range of random numbers", default=100
)
parser.add_argument(
    "--start_n_value",
    nargs="?",
    type=int,
    help="Starting point for the Range of random numbers",
    default=1,
)
parser.add_argument(
    "--end_n_value",
    nargs="?",
    type=int,
    help="Ending point for the Range of random numbers",
    default=1000000,
)
parser.add_argument(
    "--criteria", nargs="?", type=str, help="Receive a json string as an input"
)
parser.add_argument(
    "--module", nargs="?", type=str, help="Module where reside the sa models"
)
parser.add_argument(
    "--model", nargs="?", type=str, help="The specific model to pass the criteria"
)
parser.add_argument(
    "--type_scalar", nargs="?", type=str, help="Scalar List, One or None"
)
parser.add_argument(
    "--test_criteria_query",
    action="store_true",
    help="Test the criteria query processor",
)
parser.add_argument(
    "--test_ex_query",
    action="store_true",
    help="Test the shortcut method ex_query for processing the criteria",
)
...
if args.populate_db:
    ...
    engine.connect()
    data = []
    fake_year = random_numeric_data(10, 2000, 2000 + 50, ntype=int)
    ...
    with Session(engine) as session:
        session.execute(insert(Exchange), data)
        session.commit()
...
if args.test_ex_query:
    ...
    d = None
    with Session(engine) as session:
        d = ex_query(session, args.module, args.model, criteria, robj=args.type_scalar)
    if d:
        ddi = d.__dict__
        ddi.pop("_sa_instance_state", None)
        print(ddi)
